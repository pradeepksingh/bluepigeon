...
if (request.getParameterMap().containsKey("amenity_id")) {
	...
	if (amenity_id > 0) {
		amenity_stage_list = new BuilderProjectAmenityStagesDAO().getStateByAmenityId(amenity_id);
...
$("#searchamenityId").change(function(){
	window.location.href = "${baseUrl}/createprojects/builder-project-amenity-stages.jsp?amenity_id="+$("#searchamenityId").val();
});
...
function editProjectAmenityStages(amenity_stage_id) {
	...
	$.get("${baseUrl}/createprojects/editprojectamenitystage.jsp?amenity_stage_id="+amenity_stage_id,{ }, function(data){
		$("#modalarea").html(data);
