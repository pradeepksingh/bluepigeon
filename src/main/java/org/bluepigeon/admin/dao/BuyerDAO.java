...
//		newsession1.close();
	...
	public Long getTotalBuyersByBuilderId(int builderId){
		Long totalBuyers = (long) 0;
		String hql = "select COUNT(b.id) from Buyer as b where b.builder.id = :builder_id and b.isDeleted = 0 and b.isPrimary=1 and b.status=0";
		HibernateUtil hibernateUtil = new HibernateUtil();
		Session session = hibernateUtil.openSession();
		Query query = session.createQuery(hql);
	...
	public Long getTotalBuyersByEmpId(int empId){
		Long totalBuyers = (long) 0;
		String hql = "select COUNT(buy.id) from buyer as buy left join  builder_project as project on buy.project_id = project.id left join allot_project as ap on ap.project_id = project.id where ap.emp_id = :emp_id and buy.is_deleted = 0 and buy.is_primary=1 and buy.status=0";
		HibernateUtil hibernateUtil = new HibernateUtil();
		Session session = hibernateUtil.openSession();
